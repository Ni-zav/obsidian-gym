...
class ObsidianGym extends Plugin {
  ...
  renderTimer(root){const box=root.createDiv({cls:"gym-timer"}),display=box.createDiv({cls:"gym-timer-display"}),controls=box.createDiv({cls:"gym-actions"});[30,60,90,120].forEach(s=>button(controls,s+"s",()=>this.timer.start(s)));button(controls,"Stopwatch",()=>this.timer.stopwatch());button(controls,"Pause / resume",()=>{const s=this.timer.snapshot();s.paused?this.timer.resume():this.timer.pause();});button(controls,"Reset",()=>this.timer.reset());const update=()=>{if(!display.isConnected){clearInterval(handle);return;}const s=this.timer.snapshot();display.setText((s.mode==="stopwatch"?"Stopwatch ":"Rest ")+formatSeconds(s.seconds));};const handle=setInterval(update,250);update();}
