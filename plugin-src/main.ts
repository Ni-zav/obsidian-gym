const {
  Plugin, PluginSettingTab, Setting, Modal, FuzzySuggestModal, Notice, TFile, Component, moment
} = require("obsidian");
...
function norm(v){ if(typeof v!=="string") return ""; return v.replace(/\\/g,"/").trim().replace(/^\/+|\/+$/g,""); }
...
class Index {
  ...
  sessionsList(){ return [...this.sessions.values()].sort((a,b)=>new Date(b.fm.started_at||b.fm.date||0)-new Date(a.fm.started_at||a.fm.date||0)); }
  ...
  workoutLogs(id,events=true){ return [...(this.byWorkout.get(String(id))?.values()||[])].filter(r=>events||!r.eventType).sort((a,b)=>new Date(a.performedAt||0)-new Date(b.performedAt||0)); }
  history(id,name){
    ...
    return [...m.values()].sort((a,b)=>new Date(a.performedAt||0)-new Date(b.performedAt||0));
  }
  latest(id,name){ const h=this.history(id,name);return h.at(-1)||null; }
  allLogs(){ return [...this.logs.values()].filter(r=>!r.eventType).sort((a,b)=>new Date(a.performedAt||0)-new Date(b.performedAt||0)); }
}
...
class Gym {
  ...
  async log(file,p){
    return this.queue(file,async()=>{const sf=this.index.fm(file);if(sf.status==="completed")throw new Error("Workout is completed.");const h=this.index.history(p.exercise_id,p.exercise).map(x=>x.fm),pr=p.set_type==="warmup"?[]:this.prs(p,h),existing=this.index.workoutLogs(sf.id,false);
      const fm={schema_version:3,id:uuid(),workout_id:String(sf.id),set_index:existing.length+1,exercise_id:String(p.exercise_id),exercise:p.exercise,performed_at:now(),tracking_mode:p.tracking_mode,set_type:p.set_type,effort:p.effort??null,note:p.note||"",prs:pr,tags:["exercise","log","set"]};
  ...
  async createRoutine(x){const path=join(this.plugin.settings.workoutTemplatesRoot,"gym",x.name.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"")+".md");if(this.app.vault.getAbstractFileByPath(path))throw new Error("Routine already exists.");return this.create(path,{schema_version:3,workout_title:x.name,exercise_plan:x.plan,workout_type:x.type,workout_place:x.place,tags:["workout"]},"# "+x.name+"\n\n"+FENCE+"obsidian-gym-routine\n"+FENCE);}
  ...
  async queue(file,fn){const key=String(this.index.fm(file).id||file.path),prev=this.queues.get(key)||Promise.resolve(),next=prev.catch(()=>{}).then(fn);this.queues.set(key,next.then(()=>{},()=>{}));try{return await next;}finally{if(this.queues.get(key)===next)this.queues.delete(key);}}
}
...
class ObsidianGym extends Plugin {
  ...
  registerRenderers(){
    ...
    reg("obsidian-gym-home",this.renderHome);reg("obsidian-gym-session",this.renderSession);reg("obsidian-gym-exercise",this.renderExercise);reg("obsidian-gym-analytics",this.renderAnalytics);reg("obsidian-gym-recovery",this.renderRecovery);reg("obsidian-gym-routine",this.renderRoutine);reg("obsidian-gym-log",this.renderLog);
  ...
  async migrateV3(){
    ...
    let changed=0;for(const file of files){const f=this.index.fm(file);if(Number(f.schema_version||0)>=3)continue;const rel=file.path.replace(/^\/+/,"");const bp=join(backup,rel);await this.gym.ensureFolder(parent(bp));try{await this.app.vault.copy(file,bp);}catch{}
      if(inside(file.path,this.settings.exercisesRoot)&&tags(f).includes("exercise")&&!f.workout_id){const patch={schema_version:3,tracking_mode:modeOf(f),default_rest_seconds:num(f.default_rest_seconds)??this.settings.defaultRestSeconds};const dr=parseLegacy(f.default_reps??f.reps),dw=parseLegacy(f.default_weight_kg??f.weight),dd=parseLegacy(f.default_duration_seconds??f.duration);if(dr!=null)patch.default_reps=dr;if(dw!=null)patch.default_weight_kg=dw;if(dd!=null)patch.default_duration_seconds=dd;await this.gym.update(file,patch,["date","time","weight","reps","effort","timed","duration"]);changed++;}
      else if(inside(file.path,this.settings.workoutTemplatesRoot)&&tags(f).includes("workout")){await this.gym.update(file,{schema_version:3,exercise_plan:planOf(f)},["exercises","workout_order"]);changed++;}
      else if(inside(file.path,this.settings.workoutsRoot)&&file.path.includes("/Log/")){const event=f.event_type||(f.exercise==="Workout start"?"workout_start":f.exercise==="Workout end"?"workout_end":null),patch={schema_version:3,performed_at:f.performed_at||f.date||now()};if(event)patch.event_type=event;else{patch.tracking_mode=modeOf(f);patch.set_type=f.set_type||"working";if(f.weight_kg==null&&num(f.weight)!=null)patch.weight_kg=num(f.weight);if(f.duration_seconds==null&&num(f.duration)!=null)patch.duration_seconds=num(f.duration);}await this.gym.update(file,patch,["weight","duration","date"]);changed++;}
      else if(inside(file.path,this.settings.workoutsRoot)&&tags(f).includes("workout")){await this.gym.update(file,{schema_version:3,exercise_plan:planOf(f)},["exercises","workout_order"]);await this.gym.recalc(file);changed++;}
    }
