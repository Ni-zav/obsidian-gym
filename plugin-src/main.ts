...
class Gym {
  ...
  async undo(file){return this.queue(file,async()=>{const a=this.index.workoutLogs(this.index.fm(file).id,false),l=a.at(-1);if(!l)return false;const p=l.file.path;await this.app.vault.trash(l.file,false);this.index.remove(p);await this.recalc(file);return true;});}
